...
def simulate_trading(starting_balance, number_of_trades, risk_reward_ratio):
    ...
    for _ in range(number_of_trades):
        balance += balance * risk_reward_ratio if random.choice([True, False]) else -balance * risk_reward_ratio
...
plt.title('Average Trader Balance Over 10000 Trades (1000 Simulations) 1:1 RR', fontsize=14)
